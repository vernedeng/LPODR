...
RoutingProtocol::SetPosAndVelo(RreqHeader & rreqHeader)
{
  ...
  rreqHeader.SetPosx(pos.x);
  rreqHeader.SetPosy(pos.y);
  rreqHeader.SetVelo(vel);
  rreqHeader.SetDirc(dirc);
}

void
RoutingProtocol::SetPosAndVelo(RrepHeader & rrepHeader)
{
  uint64_t vel,dirc;
  Vector velo = GetMyVelo();
  Vector pos = GetMyPos();
  ...
  VelToPolar(velo.x,velo.y,vel,dirc);
  rrepHeader.SetPosx(pos.x);
  rrepHeader.SetPosy(pos.y);
  rrepHeader.SetVelo(vel);
...
Vector
RoutingProtocol::GetMyVelo()
{
  Vector vel;
  Ptr<MobilityModel> MM = m_ipv4->GetObject<MobilityModel> ();
  vel.x = MM->GetVelocity().x;
  vel.y = MM->GetVelocity().y;
  return vel;
}
...
RoutingProtocol::SendRequest (Ipv4Address dst)
{
  ...
  {
    std::cout<<"position is "<<rreqHeader.GetPosx()<<" "<<rreqHeader.GetPosy()<<" velocity is: "<<rreqHeader.GetVelo()<<" "<<rreqHeader.GetDirc()<<std::endl;
  }*/
...
RoutingProtocol::SendHello ()
{
  ...
  {
    Ptr<MobilityModel> MM = m_ipv4->GetObject<MobilityModel> ();
    double velx = MM->GetVelocity().x;
    double vely = MM->GetVelocity().y;
    double vel = sqrt(velx*velx + vely*vely);
    double dirc = atan(vely/velx);
    dirc = dirc*180/3.1415;
    if(velx>0 && vely<0) dirc+=360;
    else if(velx<0) dirc+=180;
    std::cout<<"my velocity is "<<velx<<" "<<vely<<" "<<std::endl;
    std::cout<<"total velocity in double "<<vel<<" "<<dirc<<std::endl; 
    uint64_t v,d;
    VelToPolar(velx,vely,v,d);
    std::cout<<"total velocity in polar "<<v<<" "<<d<<std::endl;   
    PolarToVel(v,d,velx,vely);
    std::cout<<"after function my velocity is "<<velx<<" "<<vely<<" "<<std::endl;
